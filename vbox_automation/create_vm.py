...
def set_vm_config(vm_name, path_to_iso, nb_cpu, memory_mb, disk_size_gb):
    ...
    try:
        ...
        subprocess.run(["VBoxManage", "createhd", "--filename", f"C:\{vm_name}/{vm_name}.vdi", "--size", str(disk_size_mb)])

        # Création du disque dur virtuel
        subprocess.run(["VBoxManage", "storagectl", vm_name, "--name", "SATA Controller", "--add", "sata", "--bootable", "on"])
        subprocess.run(["VBoxManage", "storageattach", vm_name, "--storagectl", "SATA Controller", "--port", "0", "--device", "0", "--type", "hdd", "--medium", f"C:\{vm_name}/{vm_name}.vdi"])  
